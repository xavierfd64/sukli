...
class InstallController extends Controller
{
    ...
    public function apiCheckRequirements(Request $request): void
    {
        ...
        $this->json([
            'ok' => $ok,
            'message' => $ok ? 'System requirements satisfied.' : 'Some system requirements are not met. Check PHP version (8.2+), the pdo_mysql extension, and that the project root and storage/ folder are writable.',
            'checks' => $checks,
        ]);
    }
    ...
    public function apiFinalize(Request $request): void
    {
        ...
        $envContent = $this->buildEnvContent($db, $request);
        if (@file_put_contents(Installer::envPath(), $envContent, LOCK_EX) === false) {
            $this->json(['ok' => false, 'message' => 'Could not write the .env configuration file. Check that the project root is writable, then try again.'], 500);
        }
    ...
    private function buildEnvContent(array $db, Request $request): string
    {
